...
export const QuizPage: React.FC = () => {
  ...
  }, [currentIndex, isFlipped, quizType]);

  // --- LOGIKA PISANIA (Smart Check) ---
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
      ...
      if (!isFlipped) {
          ...
          if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); handleGrade(mode === 'srs' ? Grade.Good : 5); }
      }
  }, [loading, finished, isFlipped, mode, currentIndex, canUndo, previousCardState, quizType, userAnswer]);
  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto', textAlign: 'center', paddingTop: 20 }}>
             ...
             {isFlipped && (
                <div style={{ marginTop: 10 }}>
                    ...
                    <Row gutter={16}>
                        ...
                        <Col span={8}><Button block size="large" onClick={() => handleGrade(Grade.Hard)}>Literówka (2)</Button></Col>
                        <Col span={8}><Button type="primary" block size="large" onClick={() => handleGrade(Grade.Good)} style={checkResult === 'correct' ? {boxShadow: '0 0 10px #52c41a'} : {}}>Idealnie (3)</Button></Col>
                    </Row>
                </div>
             )}
        ...
        <>
            ...
            <div style={{ marginTop: 30 }}>
                ...
                <Row gutter={16}>
                    ...
                    <>
                        <Col span={12}><Button danger block size="large" onClick={() => handleGrade(0)}>Źle</Button></Col>
                        <Col span={12}><Button type="primary" block size="large" onClick={() => handleGrade(5)}>Dobrze</Button></Col>
                    </>
                    )}
                </Row>
                )}
      ...
      <div style={{marginTop: 20, fontSize: 12, color: '#999'}}>
          {quizType === 'writing' ? "Wpisz odpowiedź i naciśnij Enter." : "Spacja (Pokaż), 1/2/3 (Ocena)"}
      </div>
    </div>
  );
};